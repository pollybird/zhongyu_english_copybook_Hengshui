...
class CopybookGenerator(QMainWindow):
    ...
    def new_project(self):
        ...
        # 重置所有参数到默认值
        self.text_content = ''
        self.font_size = 18  # 默认字号为18
        self.letter_spacing = 0
        self.position_x = 0
        self.position_y = 0
        self.line_type = '四线三格'
        self.generate_mode = '描红'
        self.custom_headers = ['班级', '姓名', '学号']
        self.title = '英文字帖'
        self.current_page = 0  # 当前页码
        self.total_pages = 1  # 总页数
        self.show_page_number = True  # 是否显示页码
        self.current_file = None  # 重置当前文件路径
    ...
    def save_project_as(self):
        ...
        if file_path:
            ...
            QMessageBox.information(self, '保存成功', '工程文件保存成功！')
...
'''
    ...
    def save_project(self):
        ...
        if self.current_file:
            ...
            # 否则弹出保存对话框
            file_path, _ = QFileDialog.getSaveFileName(self, '保存工程', '', '字帖工程文件 (*.zyecb)')
            if not file_path:
                return
            # 确保文件扩展名正确
            if not file_path.endswith('.zyecb'):
    ...
    def export_pdf(self):
        file_path, _ = QFileDialog.getSaveFileName(self, '导出PDF', '', 'PDF文件 (*.pdf)')
        if file_path:
            # 确保文件扩展名正确
